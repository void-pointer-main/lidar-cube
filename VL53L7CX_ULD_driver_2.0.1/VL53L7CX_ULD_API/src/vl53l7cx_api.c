...
/* ! vl53l7cx_get_ranging_data HAS BEEN MODIFIED IN THIS FILE ! */
		...
		uint8_t					expected_value)
{
	...
	// status |= _vl53l7cx_poll_for_answer(p_dev, size 1, pos 0, addr 0x21, mask 0x10, exp 0x10);

	do {
		status |= VL53L7CX_RdMulti(&(p_dev->platform), address,
				p_dev->temp_buffer, size);
		status |= VL53L7CX_WaitMs(&(p_dev->platform), 20);
		...
		uint16_t		        i2c_address)
{
	...
	status |= VL53L7CX_WrByte(&(p_dev->platform), 0x7fff, 0x00);
	// status |= VL53L7CX_WrByte(&(p_dev->platform), 0x4, (uint8_t)(i2c_address >> 1)); <- Why would ST do this -_-
	status |= VL53L7CX_WrByte(&(p_dev->platform), 0x4, (uint8_t)i2c_address);
             ...
             < (uint32_t)p_dev->data_read_size; i+=(uint32_t)4)
	{
		...
		switch(bh_ptr->idx){
			...
			case VL53L7CX_DISTANCE_IDX:
				...
				&(p_dev->temp_buffer[i + (uint32_t)4]), sizeof(p_results->distance_mm));//msize);
				break;
#endif
#ifndef VL53L7CX_DISABLE_REFLECTANCE_PERCENT
			...
			case VL53L7CX_TARGET_STATUS_IDX:
				...
				&(p_dev->temp_buffer[i + (uint32_t)4]), sizeof(p_results->target_status));//msize);
				break; // bad write here, msize is set to 76, should be 64. 76-64 = 12 = 3 * 4?
#endif
#ifndef VL53L7CX_DISABLE_MOTION_INDICATOR
