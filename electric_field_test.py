...
MAX_INTENSITY = 5.
def draw_display(screen, display, charges, start_r, start_c, cell_size):
    for r in range(display.resolution):
        for c in range(display.resolution):
            point = display.pixel_vectors[r][c]
            intensity = intensity_magnitude_at_point(point, charges, gain=2,exponent=2)
...
while running:
    ...
    if payload is not None and len(payload) == NUM_ROWS * NUM_COLS * 2:
        ...
        for i in range(len(displays)):
            draw_display(screen, displays[i], charges, SCREEN_POSITIONS[i][0], SCREEN_POSITIONS[i][1], CELL_SIZE)
        print(time.time()-st)
